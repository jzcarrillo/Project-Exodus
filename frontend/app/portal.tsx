...
async function api(action:string,data:object={}){
  ...
  if(!r.ok){
    const msg=Array.isArray(value.message)?value.message.join(', '):(value.message||value.error||'Unable to save. Try again.');
    throw new Error(msg);
  }
  return value;
}
          ...
          {!service ? (
            <div className="page-heading">
              <div>
                ...
                <h1>{title}</h1>
                <p>{view==='Overview'?'Everything you need for your next step, in one place.':subtitle[view]}</p>
              </div>
              <button className="primary" onClick={()=>navigate('All services')}><Plus size={18}/> New application</button>
            </div>
          ) : (
            <div className="flex items-center justify-between gap-4 mb-4">
              <button
                ...
                className="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-[#004de6] hover:text-[#003ecb] transition-colors cursor-pointer py-1"
              >
                <ArrowLeft size={16}/> Back to All services
              </button>
              <div className="text-xs text-slate-500 font-medium">
                {selected ? `Editing draft ${shortId(selected.id)}` : 'New application journey'}
              </div>
            </div>
          )}
